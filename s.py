...
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", default="your-api-key")
...
CLAUDE_API_KEY = os.environ.get("CLAUDE_API_KEY", default="")
...
def run_grpo_training(
    ...
    if rag_learning and rag_builder is not None and rag_index_builder is not None:
        ...
        if not os.path.exists(level1_dir) or not os.path.exists(level2_dir):
            print("\n" + "=" * 70)
        ...
        for batch_idx in range(num_batches):
            ...
            if rag_learning and rag_builder is not None and rag_index_builder is not None:
                ...
                try:
                    learned_count = 0
                    for rollout in rollouts:
                        if rollout.get("reward", 0) >= 0.8:  # 只学习成功的
                            app_name = rollout.get("app", "")
                            instruction = rollout.get("problem", "")
                            trajectories = rollout.get("trajectories", [])

                            if app_name and trajectories:
                                trajectory = trajectories[0].get("trajectory", [])
