...
class InfoPool:
    ...
    experiences: dict = field(default_factory=dict)  # 新增：存储 experiences
    rag_knowledge: str = ""
    rag_selected_app: str = None
...
class Manager(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        # 添加 RAG 知识（如果有）
        if info_pool.rag_knowledge and len(info_pool.rag_knowledge) > 0:
            prompt += "### RAG Knowledge from Similar Past Tasks ###\n"
            prompt += "Historical knowledge retrieved from similar task executions:\n\n"
            prompt += info_pool.rag_knowledge + "\n\n"
            prompt += (
                "Based on Knowledge, infer implicit information from the user's instruction:\n"
                "- **Preferred Apps**: Identify which apps the user typically uses for this type of task\n"
                "- **Favorite Content**: Identify specific books, authors, genres, or other content the user has shown preference for\n"
                "- **Usage Patterns**: Recognize the user's typical workflow and shortcuts they prefer\n\n"
                "When the user gives a vague instruction (e.g., 'Read the books I like', 'Open my favorite app','play my song'), "
                "use the Knowledge below to infer:\n"
                "1. The most likely app to use\n"
                "2. Probable specific content (book titles, categories, etc.) based on history\n"
                "3. The preferred approach based on past patterns\n\n"
            )

        # 添加 experiences 到 prompt（只有当 experiences 不为空时）
        if info_pool.experiences and len(info_pool.experiences) > 0:
            prompt += "### Relevant Experiences from Past Tasks ###\n"
            prompt += (
                "Based on past interactions and experiences, infer implicit information from the user's instruction:\n"
                "- **Preferred Apps**: Identify which apps the user typically uses for this type of task\n"
                "- **Favorite Content**: Identify specific books, authors, genres, or other content the user has shown preference for\n"
                "- **Usage Patterns**: Recognize the user's typical workflow and shortcuts they prefer\n\n"
                "When the user gives a vague instruction (e.g., 'Read the books I like', 'Open my favorite app', "
                "use the experiences below to infer:\n"
                "1. The most likely app to use\n"
                "2. Probable specific content (book titles, categories, etc.) based on history\n"
                "3. The preferred approach based on past patterns\n\n"
            )
            for exp_id, exp_content in info_pool.experiences.items():
                prompt += f"[{exp_id}] {exp_content}\n"
            prompt += "\n"
            print(prompt)
    ...
    # parse_response 添加 None 检查
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: Manager received None response from API")
            return {
                "thought": "API call failed - response is None",
                "plan": "Unable to generate plan due to API failure",
                "current_subgoal": "Retry or skip current step",
                "error": True
            }
        ...
        try:
            ...
            print(f"Warning: Manager parse_response failed: {e}")
            return {
...
class Operator(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        # 添加 RAG 知识
        if info_pool.rag_knowledge and len(info_pool.rag_knowledge) > 0:
            prompt += "### RAG Knowledge ###\n"
            prompt += info_pool.rag_knowledge + "\n\n"
            prompt += "NOTE: UI coordinates in RAG knowledge are for reference only. "
            prompt += "Always use current screen detection for accurate coordinates.\n\n"

        # 添加 experiences（只有当有 experiences 时）
        if info_pool.experiences and len(info_pool.experiences) > 0:
            prompt += "### Relevant Experiences ###\n"
            prompt += "Here are some relevant experiences from past tasks:\n"
            for exp_id, exp_content in info_pool.experiences.items():
                prompt += f"[{exp_id}] {exp_content}\n"
            prompt += "\n"
        ...
        if info_pool.action_history != []:
            ...
            if len(latest_outcomes) >= 2 and len(action_log_strs) >= 2:
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: Operator received None response from API")
            return {
                "thought": "API call failed - response is None",
        ...
        try:
            ...
            print(f"Warning: Operator parse_response failed: {e}")
            return {
                "thought": f"Parse error: {str(e)}",
                "action": '{"name": "Wait", "arguments": null}',
                "description": "Waiting due to parse error",
                "error": True
            }
...
class ActionReflector(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ActionReflector received None response from API")
            return {
                "outcome": "C",  # 标记为失败但无变化
                "error_description": "API call failed - response is None",
                "progress_status": "Unable to determine progress due to API failure",
                "error": True
            }
        ...
        try:
            outcome = response.split("### Outcome ###")[-1].split("### Error Description ###")[0].replace("\n",
                                                                                                          " ").replace(
                "  ", " ").strip()
            error_description = response.split("### Error Description ###")[-1].split("### Progress Status ###")[
                0].replace("\n", " ").replace("  ", " ").strip()
            ...
            print(f"Warning: ActionReflector parse_response failed: {e}")
            return {
                "outcome": "C",
                "error_description": f"Parse error: {str(e)}",
                "progress_status": "Unable to parse progress",
                "error": True
            }
...
class Notetaker(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: Notetaker received None response from API")
            return {
                "important_notes": "Unable to take notes due to API failure",
                "error": True
            }
        ...
        try:
            ...
            print(f"Warning: Notetaker parse_response failed: {e}")
            return {
                "important_notes": f"Parse error: {str(e)}",
                "error": True
            }
...
class ExperienceReflectorShortCut(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ExperienceReflectorShortCut received None response from API")
            return {"new_shortcut": "None", "error": True}

        try:
            new_shortcut = response.split("### New Shortcut ###")[-1].replace("\n", " ").replace("  ", " ").strip()
            return {"new_shortcut": new_shortcut}
        except Exception as e:
            print(f"Warning: ExperienceReflectorShortCut parse_response failed: {e}")
            return {"new_shortcut": "None", "error": True}
...
class ExperienceReflectorTips(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ExperienceReflectorTips received None response from API")
            return {"updated_tips": "", "error": True}

        try:
            updated_tips = response.split("### Updated Tips ###")[-1].replace("\n", " ").replace("  ", " ").strip()
            return {"updated_tips": updated_tips}
        except Exception as e:
            print(f"Warning: ExperienceReflectorTips parse_response failed: {e}")
            return {"updated_tips": "", "error": True}
...
class ExperienceRetrieverShortCut(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ExperienceRetrieverShortCut received None response from API")
            return {"selected_shortcut_names": [], "error": True}
        ...
        try:
            ...
            print(f"Warning: ExperienceRetrieverShortCut parse_response failed: {e}")
            return {"selected_shortcut_names": [], "error": True}
...
class ExperienceRetrieverTips(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ExperienceRetrieverTips received None response from API")
            return {"selected_tips": "", "error": True}

        try:
            selected_tips = response.split("### Selected Tips ###")[-1].replace("\n", " ").replace("  ", " ").strip()
            return {"selected_tips": selected_tips}
        except Exception as e:
            print(f"Warning: ExperienceRetrieverTips parse_response failed: {e}")
            return {"selected_tips": "", "error": True}


class ExperienceRetriever(BaseAgent):
    """检索与当前任务相关的 experience"""

    def init_chat(self) -> list:
        operation_history = []
        system_prompt = (
            "You are a helpful AI assistant specializing in mobile phone operations. "
            "Your goal is to select relevant experiences from previous tasks that can "
            "help with the current task."
        )
        operation_history.append(["system", [{"type": "text", "text": system_prompt}]])
        return operation_history

    def get_prompt(self, instruction: str, experiences: dict) -> str:
        prompt = "### Available Experiences from Past Tasks ###\n"
        prompt += "Here are experiences collected from previous tasks:\n\n"
        ...
        prompt += "---\n"
        prompt += (
            "Carefully examine the experiences above and identify which ones are "
            "relevant to the current task. Consider:\n"
            "- Does the experience involve similar apps or operations?\n"
            "- Does it provide useful strategies or patterns applicable to this task?\n"
            "- Does it contain error handling or tips relevant to this scenario?\n\n"
            "Select experiences that can genuinely help accomplish the current task. "
            "Remove experiences that are task-specific to other domains or irrelevant.\n\n"
        )

        prompt += "Provide your output in the following format:\n\n"
        prompt += "### Selected Experiences ###\n"
        prompt += (
            "List the IDs of selected experiences as a JSON array: [\"exp_id_1\", \"exp_id_2\", ...]\n"
            "If no experiences are relevant, return an empty array: []\n"
        )
    ...
    def parse_response(self, response: str) -> dict:
        # 处理 None 响应（API 调用失败时）
        if response is None:
            print("Warning: ExperienceRetriever received None response from API")
            return {"selected_experience_ids": [], "error": True}

        try:
            selected_exp_str = response.split("### Selected Experiences ###")[-1].strip()
            try:
                selected_exp_ids = extract_json_object(selected_exp_str, json_type="list")
                if selected_exp_ids is None:
                    selected_exp_ids = []
                selected_exp_ids = [str(exp_id).strip() for exp_id in selected_exp_ids]
            except:
                selected_exp_ids = []
            return {"selected_experience_ids": selected_exp_ids}
        except Exception as e:
            print(f"Warning: ExperienceRetriever parse_response failed: {e}")
            return {"selected_experience_ids": [], "error": True}